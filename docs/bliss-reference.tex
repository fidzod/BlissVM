...
\begin{blissnote}[\code{r0} is volatile]
Several pseudo-instructions use \reg{r0} as a temporary and clobber it. Do not
hold a live value in \reg{r0} across a \code{PUSH}, \code{POP}, \code{CALL},
\code{RET}, or \code{ADDI}.
\end{blissnote}
...
\code{CALL label}   & \code{PUSH r14} $\cdot$ \code{BAL r14, label} & \code{r0} \\
\code{RET}          & \code{POP r14} $\cdot$ \code{BALR r0, r14} & \code{r0} \\
...
\noindent \code{LI} is implemented today; the stack and call family are Phase~1
of the road to the interactive shell.
...
\reg{r14}        & Link register (\code{LR}) & Saved by \code{CALL} \\
