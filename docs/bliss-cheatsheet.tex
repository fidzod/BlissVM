...
\code{r14 LR}  & link register --- return address (caller-saved) \\
...
\code{CALL lbl}    & \code{BAL r14,lbl} \quad(clobbers \code{r14}) \\
\code{RET}         & \code{BALR r0,r14} \\
\code{JMP lbl/rs}  & \code{BAL r0,lbl} / \code{BALR r0,rs} \\
\code{ADDI rd,imm} & \code{LDI16 r0,imm};\; \code{ADD rd,rd,r0} \\
\end{tabular}
\par\vspace{0.2em}\noindent
{\footnotesize \code{LI} clobbers nothing, \code{CALL} clobbers \code{r14}, the
rest clobber \code{r0}. \code{CALL}/\code{RET} do no stack traffic: a non-leaf
function does \code{push lr} first and \code{pop lr} before \code{RET}
(same for \code{r8--r12} if used); leaf functions need neither.}
